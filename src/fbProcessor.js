...
class Db {
  ...
  addTodo({
    ...
    this.firestore.doc(`/users/3nrCmkaHwUvK1zpOLmKG/projects/${projectid}/todos/${id}`).set({
      title,
      description,
      priority,
      duedate,
  ...
  async updateTodo({
    ...
    try {
      await this.firestore
        .doc(`/users/3nrCmkaHwUvK1zpOLmKG/projects/${projectid}/todos/${id}`)
        .update({
          title,
          description,
          priority,
  ...
  async queryToday1(fn) {
    ...
    docsSnapshot.forEach((docs) => {
      const snapshot = this.firestore
        .collection(`/users/3nrCmkaHwUvK1zpOLmKG/projects/${docs.id}/todos/`)
  ...
  async getProjects(fn) {
    const snapshot = await this.firestore.collection('/users/3nrCmkaHwUvK1zpOLmKG/projects/').get();
  ...
  async queryToday2(fn) {
    ...
    Promise.all(promises).then((snapshot) => {
      if (snapshot.empty) console.log('No such documents!');
      else
        snapshot.forEach((doc) => {
          fn({ id: doc.id, ...doc.data() });
        });
  ...
  async queryProject(fn, projectid) {
    const projectRef = this.firestore.collection(
      `/users/3nrCmkaHwUvK1zpOLmKG/projects/${projectid}/todos`,
    );
  ...
  addProject({ id, title }) {
    this.firestore.collection('/users/3nrCmkaHwUvK1zpOLmKG/projects/').doc(`${id}`).set({
      title,
      counter: 0,
    });
  }
  ...
  async deleteProject(id) {
    ...
    const promises = [];
    const projectRef = this.firestore.collection(
      `/users/3nrCmkaHwUvK1zpOLmKG/projects/${id}/todos/`,
    );
    const snapshot = await projectRef.get();
    snapshot.forEach((doc) => {
      promises.push(
        this.firestore.doc(`/users/3nrCmkaHwUvK1zpOLmKG/projects/${id}/todos/${doc.id}`).delete(),
      );
    });

    Promise.all(promises).then(async (snapshot) => {
      if (snapshot.empty) console.log('No such documents!');
      else await this.firestore.doc(`/users/3nrCmkaHwUvK1zpOLmKG/projects/${id}`).delete();
    });
