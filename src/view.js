...
class View {
  ...
  renderTodo({ id, title, description, duedate, priority, projectid }) {
    ...
    const dateCtrlContainer = document.createElement('div');
  ...
  renderProject({ id, title, counter = 0 }) {
    ...
    const delButton = document.createElement('div');
    const projectsCounter = document.createElement('div');
  ...
  projectInc({ projectid }) {
    const projectNode = document.querySelector(
      `[data-id="${projectid}"] .menu-container__subitem-counter`,
    );
  ...
  renderHomeProject({ id, title, counter = 0 }) {
    const container = document.querySelector('.menu-container__home');
    const li = document.createElement('li');
    const span = document.createElement('span');
    const projectsCounter = document.createElement('div');
  ...
  findRoot(node) {
    while (!node.dataset.id) node = node.parentNode;
    return node;
  }
...
class UlTodo extends View {
  constructor() {
    super();
    this.container = document.querySelector('.todos-container');
    this.buttonsContainer = document.querySelector('.todos-container');
    this.containerClass = 'todos-container';
  }

  descriptionToggle(id) {
    const desc = document.querySelector(
      `[data-id="${id}"] .${this.containerClass}__item-description`,
    );
    if (desc.style !== '') desc.style.display = desc.style.display === 'none' ? 'flex' : 'none';
...
class Aside extends View {
  constructor() {
    ...
    this.buttonsContainer = document.querySelector('.menu-container');
...
class MainContent extends View {
  constructor() {
    ...
    this.buttonsContainer = document.querySelector('.main-content');
...
class EditForm extends View {
  constructor(todoid) {
    ...
    const priorityClass = this.todo.querySelector('.todos-container__item-date').classList.item(1);
    this.priority = +priorityClass.slice(-1);
  ...
  renderEdit() {
    ...
    const nodes = {};
    // for (const [key, item] of Object.entries(nodesConfig)) {
    //   for (const value of item)
    //     nodes[value] = document.createElement(`${key}`)
    // }
    Object.entries(nodesConfig).forEach(([key, item]) => {
      item.forEach((value) => {
        nodes[value] = document.createElement(`${key}`);
      });
    });
    ...
    nodes.inputs.append(this.titleNode, this.buttons.description);
    nodes.inputs.append(this.buttons.date, this.buttons.priority);
  ...
  togglePriority() {
    if (this.priorityNode) {
      this.priority = this.priorityNode.selectedIndex;
...
class Calendar {
  constructor(duedate) {
    if (duedate && duedate !== '') {
      this.todoDate = duedate;
      this.shiftingDate = duedate;
    } else this.todoDate = this.shiftingDate = new Date();
  ...
  create() {
    ...
    const controlsheaderContainer = document.createElement('div');
  ...
  monthShift(button = 'now') {
    if (button === 'prev') this.shiftingDate = add(this.shiftingDate, { months: -1 });
    if (button === 'next') this.shiftingDate = add(this.shiftingDate, { months: 1 });
  }
  ...
  renderDatePicker() {
    ...
    for (let i = 0; i < numOfCalendarRows; i += 1) {
      ...
      for (let j = 0; j < daysInWeek; j += 1) {
        ...
        if (!this.page[i][j].later) cell.classList.add('date-picker__date-list__cell-filler');
        if (this.page[i][j].tododate) cell.classList.add('date-picker__date-list__cell-todo');
        row.append(cell);
      }
      datelist.append(row);
    }
  ...
  renderDateHeader() {
    ...
    this.dateHeader.textContent = `${
      monthNames[this.shiftingDate.getMonth()]
    }, ${this.shiftingDate.getFullYear()}`;
  }
  ...
  initTable() {
    ...
    for (let i = 0; i < 6; i += 1) {
      ...
      for (let j = 0; j < 7; j += 1) {
        row.push({
          date: getDate(date),
          later: compareAsc(date, this.today) >= 0,
          tododate: compareAsc(date, this.todoDate) === 0,
          month: date.getMonth(),
